...
if __name__ == "__main__":
    ...
    env = nasim.make_benchmark(args.env_name,
                               ...
                               flat_actions=True,
                               flat_obs=True)
    random_agent = MyRandom(env, **vars(args))
    random_agent.train()

    print(f"\n{'-'*60}\nDone\n{'-'*60}")
    print(f"Best Policy score = {random_agent.best_score}")
    print(f"Policy saved to: {random_agent.save_path}")
    print(f"Result saved to: {result_path}")

    with open(result_path_data + "/Random.json", "w") as f:
        json.dump(results, f)

    with open(result_path_testing + "/Random_testing.json", "w") as f:
        json.dump(random_agent.testing_monitor, f)
