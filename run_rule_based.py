...
if __name__ == "__main__":
    ...
    env = nasim.make_benchmark(args.env_name,
                               ...
                               flat_actions=True,
                               flat_obs=True)
    rule_based_agent = MyRule(env, **vars(args))
    rule_based_agent.train()

    print(f"\n{'-'*60}\nDone\n{'-'*60}")
    print(f"Best Policy score = {rule_based_agent.best_score}")
    print(f"Policy saved to: {rule_based_agent.save_path}")
    print(f"Result saved to: {result_path}")

    with open(result_path_data + "/RuleBased.json", "w") as f:
        json.dump(results, f)

    with open(result_path_testing + "/RuleBased_testing.json", "w") as f:
        json.dump(rule_based_agent.testing_monitor, f)
